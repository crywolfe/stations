...
public class RandomUserBO {
    ...
    public List<User> getUserByUsername(String userName, String seed, String results) throws IOException {
        List<User> users = randomUserService.getAllUsers(seed, results);
        return users.stream().filter((user -> userName.equals(user.getLogin().getUsername()))).collect(Collectors.toList());
    };
}
